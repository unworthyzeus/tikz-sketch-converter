...
- Function sampler for expressions such as `sin(x)`, `cos(2*x)`, `0.2*x^2 - 2`, `exp(-x^2)`, with mathematical exponent precedence, implicit multiplication like `2x + 3pi`, and PGFPlots axis export modes including semilog, loglog, and polar.
- Function and PGFPlots object exports expose axis bounds, label styles, tick/legend/grid styling, reversed axes, minor ticks, and limit padding controls.
...
- Searchable, draggable TikZ object palette with concrete items such as resistors, capacitors, sources, diodes, transistors, op amps, logic gates, PGFPlots axes, error bars, constellation modulation presets including dense square QAM, OFDM pilot estimators, adaptive equalizers, Bode margin plots, scatter plots, bar charts, histograms, heatmaps, automata states, matrices, commutative diagrams, flowchart shapes, neural layers, CNN stacks, transformer blocks, pipelines, Gantt bars, Petri nets, trees, angle markers, Venn diagrams, and brace annotations.
